...
(function () {
    ...
    window.saveLabelArticle = async function () {
        ...
        if (!articleNumber) {
            window.showToast('Bitte eine Artikelnummer eingeben.');
            return;
        }
        if (!/^[0-9]{1,8}$/.test(articleNumber)) {
            window.showToast('Die Artikelnummer darf nur aus Ziffern bestehen (max. 8 Stellen).');
            return;
        }
    ...
    async function handleLabelImportFile(file) {
        ...
        if (ext === 'xlsx' || ext === 'xls') {
            ...
            window.showToast('Bitte eine gültige Excel- (.xlsx, .xls) oder CSV-Datei (.csv) auswählen.');
        }
    }
    ...
    async function parseLabelCSVFile(file) {
        ...
        reader.onload = function (e) {
            try {
                ...
                window.showToast('Fehler beim Lesen der CSV-Datei: ' + err.message);
            }
        };
        reader.readAsArrayBuffer(file);
    }
    ...
    window.confirmLabelImport = async function () {
        ...
        if (numIdx === '') {
            window.showToast('Bitte die Spalte für die Artikelnummer zuordnen.');
            return;
        }
        ...
        if (rows.length === 0) {
            window.showToast('Keine gültigen Zeilen mit Artikelnummer gefunden.');
            return;
        }
        ...
        try {
            ...
            await window.fetchLabelArticles();
            window.showToast(`${rows.length} Artikel erfolgreich importiert.`);
        } catch (err) {
            console.error('Error importing label articles:', err);
            window.showToast('Fehler beim Importieren: ' + err.message);
        }
    };
    ...
    window.printSelectedLabels = async function () {
        if (selectedLabelIds.size === 0) {
            window.showToast('Bitte mindestens einen Artikel auswählen.');
            return;
        }
        ...
        try {
            ...
            try { logo = await loadEmbeddedLogoGrayscale(); } catch (e) {
                console.warn(e);
                window.showToast('Hinweis: Logo konnte nicht geladen werden, Etiketten werden ohne Logo erzeugt.\n' + e.message);
            }
            ...
            window.showToast('Fehler beim Erzeugen der Vorschau: ' + err.message);
        }
    };
    ...
    window.downloadLabelPreviewPDF = async function () {
        ...
        try {
            ...
            window.showToast('Fehler beim Erzeugen des PDF: ' + err.message);
        }
    };
    ...
    window.printVorlageLabel = async function (vorlageId) {
        ...
        try {
            ...
            window.showToast('Fehler beim Drucken: ' + err.message);
        }
    };
    ...
    window.removeBeschriftungPage = function () {
        if (beschriftungPages.length <= 1) {
            window.showToast('Mindestens eine Seite muss vorhanden bleiben.');
            return;
        }
    ...
    window.printBeschriftung = async function () {
        ...
        if (!hasContent) {
            window.showToast('Bitte mindestens auf einer Seite einen Titel oder ein Bild angeben.');
            return;
        }
