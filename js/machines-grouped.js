...
(function () {
    ...
    function renderMachinesGrouped(targetId = 'machine-list-container') {
        if (typeof targetId !== 'string' || !targetId) {
            targetId = 'machine-list-container';
        }
        ...
        let displayMachines = allMachines.filter(m => {
            // Category Filter
            const matchCat = catFilters.includes('all') || (m.category_id != null && (catFilters.includes(m.category_id.toString()) || catFilters.includes(Number(m.category_id))));
        ...
        // All categories can serve as grouped keys
        (window.categoryList || []).forEach(cat => {
            grouped[String(cat.id)] = [];
        });
        
        displayMachines.forEach(machine => {
            const catIdStr = machine.category_id != null ? String(machine.category_id) : null;
            if (catIdStr && grouped[catIdStr]) {
                grouped[catIdStr].push(machine);
            } else {
                uncategorized.push(machine);
            }
        });

        const sortedCategories = [...(window.categoryList || [])].sort((a, b) => (a.order || a.sort_order || 0) - (b.order || b.sort_order || 0));

        let categoryCount = 0;
        sortedCategories.forEach(cat => {
            const categoryMachines = grouped[String(cat.id)] || [];
        ...
        // Uncategorized Machines Section
        if (uncategorized.length > 0) {
            ...
            uncatSection.className = 'machine-category-section';
            uncatSection.style.marginBottom = '3rem';

            // If no category sections were rendered at all, don't wrap in "Sonstige / Ohne Kategorie" toggle, just show grid
            if (categoryCount === 0) {
                const grid = document.createElement('div');
                grid.className = 'card-grid';
                grid.style.marginTop = '1.5rem';

                uncategorized.forEach(machine => {
                    grid.appendChild(createMachineCard(machine));
                });
                ...
                header.className = 'machine-category-header';
                header.innerHTML = `
                    <h2 style="font-size: 1.8rem; color: #9ca3af; margin: 0; font-family: 'Outfit', sans-serif; display: flex; align-items: center; gap: 15px;">
                        <svg class="chevron" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                            <polyline points="6 9 12 15 18 9"></polyline>
                        </svg>
                        Sonstige / Ohne Kategorie
                        <span style="background: rgba(255,255,255,0.15); padding: 4px 12px; border-radius: 12px; font-size: 1rem; color: #ffffff; font-weight: 800;">${uncategorized.length}</span>
                    </h2>
                `;
                ...
                uncatSection.appendChild(header);
                uncatSection.appendChild(grid);
        ...
        card.innerHTML = `
            ...
            <div class="card-content" style="padding: 1.25rem 1.25rem 2px 1.25rem; flex: 1; display: flex; flex-direction: column;">
                ...
                <div class="card-actions" style="margin-top: auto; display: flex; align-items: center; justify-content: center; gap: 8px; padding-top: 0.75rem; border-top: 1px solid rgba(255,255,255,0.06); margin-left: -1.25rem; margin-right: -1.25rem;">
                    ...
                    <button class="btn-history" style="flex: 1 !important; padding: 8px 10px !important; font-size: 0.85rem !important;" onclick="if(typeof event !== 'undefined' && event.stopPropagation) event.stopPropagation(); window.openHistoryModal ? window.openHistoryModal('${machine.id}') : window.showToast('Historie für ' + '${machine.id}')" title="Historie">
