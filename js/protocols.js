...
(function () {
    ...
    window.openIntakeProtocol = async function (machineId, protocolId = null) {
        ...
        try {
            ...
            window.showToast('Fehler beim Öffnen des Protokolls: ' + err.message);
        }
    };
    ...
    async function openProtocolModal(machineId, protocolId, type) {
        ...
        if (!machine) {
            window.showToast('Maschine nicht gefunden');
            return;
        }
    ...
    window.saveNewCheckpoint = function () {
        ...
        if (!description) {
            window.showToast('Bitte geben Sie eine Beschreibung ein');
            return;
        }
    ...
    window.deleteCustomCheckpoint = function (index) {
        if (typeof window.canDelete === 'function' && !window.canDelete('Prüfpunkten')) return;
    ...
    window.saveProtocol = async function () {
        ...
        if (missingComments.length > 0) {
            window.showToast('Bitte tragen Sie für alle rot markierten (defekten) Prüfpunkte eine Fehlerbeschreibung ein:\n\n- ' + missingComments.join('\n- '));
            return;
        }
    ...
    window.completeProtocol = async function () {
        ...
        if (pendingCheckpoints.length > 0) {
            window.showToast('Bitte füllen Sie alle Prüfpunkte aus, bevor Sie das Protokoll abschließen:\n\n- ' + pendingCheckpoints.slice(0, 5).join('\n- ') + (pendingCheckpoints.length > 5 ? '\n... und weitere' : ''));
            return;
        }
    ...
    async function loadProtocol(protocolId, type) {
        ...
        if (error) {
            ...
            window.showToast('Fehler beim Laden: ' + error.message);
            return;
        }
    ...
    window.generateProtocolPDF = async function (previewOpen = false) {
        try {
            ...
            if (previewOpen) {
                ...
                doc.save(fileName);
                window.showToast('PDF erfolgreich erstellt!');
            }
        } catch (err) {
            document.getElementById('pdf-gen-loading')?.remove();
            console.error('PDF generation error:', err);
            window.showToast('Fehler beim Erstellen des PDFs: ' + err.message);
        }
    };
    ...
    window.previewProtocolPDF = async function () {
        try {
            ...
            document.getElementById('pdf-preview-loading')?.remove();
            window.showToast('Fehler bei der Vorschau: ' + err.message);
        }
    };
    ...
    window.saveProtocolPDFToR2 = async function() {
        ...
        if (!protocolId) {
            window.showToast('Bitte speichern Sie das Protokoll ab, bevor Sie ein PDF erstellen.');
            return;
        }
        ...
        try {
            ...
            window.showToast('PDF erfolgreich generiert, in der Cloud gespeichert und dem Protokoll zugeordnet!');
        } catch (err) {
    ...
    window.deleteProtocol = async function(protocolId, protocolType) {
        ...
        try {
            ...
            window.showToast('Fehler beim Löschen des Protokolls: ' + (err.message || JSON.stringify(err)));
        }
    };
    ...
    window.openProtocolPDF = async function (machineId, protocolId, type) {
        try {
            ...
            window.showToast('Fehler beim Öffnen des PDFs');
        }
    };
