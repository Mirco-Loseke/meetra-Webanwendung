...
window.togglePaidStatus = async function (id, checked) {
    try {
        ...
        window.showToast('Fehler beim Aktualisieren des Zahlungsstatus.');
    }
};
...
window.deleteAccountingEntry = async function (id) {
    if (typeof window.canDelete === 'function' && !window.canDelete('Buchungen')) return;
    ...
    try {
        ...
        window.showToast('Fehler beim Löschen.');
    }
};

// --- Hilfsfunktionen für die KI-Beleganalyse ---

// Ergebnis-/Warnbanner im Modal (ersetzt die störenden window.showToast()-Popups nach der Analyse)
...
window.handleAccountingPDFDrop = async function (event) {
    if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
        ...
        if (file.type === "application/pdf" || file.type.startsWith("image/")) {
            ...
            window.showToast('Bitte nur PDF-Dateien oder Bilder hochladen.');
        }
    }
};
...
window.revertSplit = async function(accountingId, itemIdx) {
    ...
    try {
        ...
        window.showToast('Fehler beim Rückgängigmachen der Aufteilung: ' + err.message);
    }
};
...
window.submitSplit = async function() {
    ...
    try {
        ...
        window.showToast('Fehler beim Aufteilen der Position: ' + err.message);
        btn.innerHTML = 'Aufteilen bestätigen';
        btn.disabled = false;
    }
};
