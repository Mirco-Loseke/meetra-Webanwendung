...
(function () {
    ...
    // Vergleichsform für Namen: klein, ohne Umlaute, ohne Satzzeichen und
    // ohne Anrede. Ohne das scheiterte "Jörg" an "Joerg" und "für Herrn
    // Müller" an allem.
    function normName(s) {
        return String(s || '').toLowerCase()
            .replace(/ä/g, 'ae').replace(/ö/g, 'oe').replace(/ü/g, 'ue').replace(/ß/g, 'ss')
            .replace(/[^a-z0-9 ]/g, ' ')
            .replace(/\b(fuer|den|dem|die|der|das|herr|herrn|frau|kollege|kollegin|an|von|bitte)\b/g, ' ')
            .replace(/\s+/g, ' ').trim();
    }

    // Ordnet EINEN genannten Vor-/Nachnamen einem Benutzer zu.
    function matchUserId(hint) {
        const ids = matchUserIds(hint);
        return ids.length ? ids[0] : null;
    }

    // Ordnet ALLE genannten Namen zu. Die KI liefert im assignee_hint auch
    // mal "Timo und Mirco" oder "Timo, Jörg" — vorher fiel dann alles durch,
    // weil der ganze Text als ein einziger Name gesucht wurde.
    function matchUserIds(hint) {
        const roh = String(hint || '').trim();
        if (!roh) return [];
        const list = (window.userList || []).filter(u => u && u.name);
        if (!list.length) return [];

        const teileVonName = u => normName(u.name).split(' ').filter(Boolean);
        const treffer = [];

        roh.split(/\s*(?:,|;|\bund\b|\bsowie\b|&|\+|\/)\s*/i)
            .map(t => normName(t)).filter(Boolean)
            .forEach(t => {
                // exakter Name, dann einzelner Namensteil (Vorname genügt),
                // dann Wortanfang, zuletzt Initialen
                let hit = list.find(u => normName(u.name) === t);
                if (!hit) hit = list.find(u => teileVonName(u).includes(t));
                if (!hit && t.length >= 3) hit = list.find(u => teileVonName(u).some(w => w.startsWith(t)));
                if (!hit) hit = list.find(u => normName(u.initials) === t);
                if (hit && !treffer.some(id => String(id) === String(hit.id))) treffer.push(hit.id);
            });
            ...
            <div class="modal-content glass-card" style="max-width: 720px; width: 96%; padding: 1.75rem; display: flex; flex-direction: column; max-height: 92vh; border: 1px solid rgba(255,255,255,0.12);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem; flex-shrink:0;">
                    <h2 style="margin:0; font-size:1.35rem; color:#fff; display:flex; align-items:center; gap:10px;">
                        <span style="font-size:1.4rem;">✨</span> <span id="ai-capture-title">KI-Erfassung</span>
                    </h2>
    ...
    // Bereich, in dem die Erfassung gerade läuft:
    //   'aufgaben'  – nur Aufgaben (Knopf in der Aufgaben-Ansicht)
    //   'vorgaenge' – nur Vorgänge (Knopf in der Vorgänge-Ansicht)
    //   'alles'     – beides (alter Aufruf ohne Angabe)
    // Der Bereich kürzt den Prompt: Wer unter Aufgaben steht, braucht der KI
    ...
    window.openAiCaptureModal = function (bereich) {
        ...
        if (titel) {
            titel.textContent = captureBereich === 'aufgaben' ? 'KI-Erfassung — Aufgaben'
                : captureBereich === 'vorgaenge' ? 'KI-Erfassung — Vorgänge'
                    : 'KI-Erfassung';
        }
    ...
    window.runAiCapture = async function () {
        ...
        // Prompt-Bausteine. Je nach Bereich wird nur das zusammengesetzt, was
        // wirklich gebraucht wird — unter „Aufgaben" fällt alles zu Vorgängen
        // weg und umgekehrt. Das spart bei jeder Anfrage mehrere hundert Token
        // und die KI kann die falsche Sorte gar nicht erst zurückgeben.
        const willAufgaben = captureBereich !== 'vorgaenge';
        const willVorgaenge = captureBereich !== 'aufgaben';

        const schemaAufgaben = `  "aufgaben": [
    { "title": "kurzer Titel", "description": "optional, Details",
      ...
      "assignee_hint": "Name(n) der Person(en), die es tun sollen, falls genannt — mehrere mit Komma trennen, sonst leer",
      "subtasks": [ { "title": "Arbeitsschritt", "supergroup": "eine Übergruppe", "suggested": false } ] }
  ]`;

        const schemaVorgaenge = `  "vorgaenge": [
    { "title": "kurzer Titel", "process_type": "einer aus [note, call, appointment, repair, maintenance, offer, order, complaint, other]",
      ...
      "steps": [ "kurzer Schritt-Text" ] }
        ...
        const teile = [];
        teile.push(`Du bist ein Assistent für eine Maschinen-Service-Firma. Wandle die folgende freie Beschreibung in strukturierte Daten um und antworte AUSSCHLIESSLICH mit einem JSON-Objekt (Deutsch), ohne Erklärtext.

Schema:
{
${[willAufgaben && schemaAufgaben, willVorgaenge && schemaVorgaenge].filter(Boolean).join(',\n')}
}`);

        if (captureBereich === 'aufgaben') {
            teile.push('Es werden AUSSCHLIESSLICH Aufgaben erfasst. Auch Telefonate, Angebote oder Bestellungen als Aufgabe anlegen. Gib kein Feld "vorgaenge" zurück.');
        } else if (captureBereich === 'vorgaenge') {
            teile.push('Es werden AUSSCHLIESSLICH Vorgänge erfasst. Auch Werkstattarbeiten als Vorgang anlegen (process_type "repair" bzw. "maintenance"). Gib kein Feld "aufgaben" zurück.');
        }

        if (willVorgaenge) {
            teile.push(`process_type richtig wählen (wichtig):
...
- Ein konkretes Datum ("14.08.") übernehmen, Jahr ergänzen.`);
        ...
        if (captureBereich === 'alles') {
            teile.push(`Unterscheidung AUFGABE vs. VORGANG (wichtig!):
...
- VORGANG = Kommunikation oder ein Geschäftsereignis rund um eine Maschine, KEINE Werkstattarbeit.
  ...
  "Kunde anrufen wegen Termin" ("call"), "Ersatzteil bestellen" ("order"), "Reklamation" ("complaint").`);
        ...
        if (willAufgaben) teile.push(`Übergruppen (supergroup) je subtask — wähle die thematisch passendste aus dieser Liste, sonst "Allgemein":
[${groupNames.join(', ')}].
...
- Maschinen/Namen/Nummern/Personen NICHT erfinden. Leere Listen sind erlaubt. Wenn keine Maschine genannt ist, lasse machine_hint leer (nicht raten).`);
        ...
        if (willAufgaben) {
            teile.push('- Denke als erfahrener Servicetechniker mit: Ergänze über die genannten Schritte hinaus sinnvolle, üblicherweise dazugehörige Unteraufgaben (z.B. Sicht-/Funktionsprüfung, Probelauf, Doku/Fotos, Ersatzteil prüfen, Entsorgung, Reinigung). Solche selbst ergänzten Schritte mit "suggested": true markieren; ausdrücklich genannte Schritte mit "suggested": false. Halte Vorschläge realistisch und knapp (max. 3-5 zusätzliche), keine erfundenen Fakten (keine erfundenen Maschinen, Namen, Nummern, Termine).');
        }

        if (willVorgaenge) {
            teile.push(`Schritte (steps) je Vorgang — nur wirklich passende, logische Arbeitsschritte für GENAU diesen Vorgangstyp vorschlagen, keine generischen Floskeln:
...
- repair/maintenance: z.B. "Termin mit Kunde abstimmen", "Ersatzteile klären", "Rückmeldung nach Erledigung".
...
Maximal 4 Schritte, nur wenn sie inhaltlich wirklich zum genannten Vorgang passen. Keine erfundenen Fakten, keine Wiederholung des Titels als Schritt. Lieber ein leeres Array als unpassende/erzwungene Schritte.`);
        ...
        try {
            ...
            // KI trotz Anweisung welche zurückgibt.
            lastResult = {
    ...
    function renderPreview() {
        ...
        if (aufgaben.length) {
            ...
            aufgaben.forEach((a, i) => {
                ...
                const aUserIds = matchUserIds(a.assignee_hint);
                ...
                <div class="ai-cap-card" data-kind="task" data-index="${i}" data-assign="${mode}" style="background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:14px; padding:1rem; margin-bottom:0.75rem;">
                    ...
                    <div style="margin-bottom:0.5rem;">
                        ...
                        ${assigneeControl(`aicap-asg-t-${i}`, aUserIds)}
                    </div>
    ...
    window.saveAiCaptureResults = async function () {
        ...
        if (chosen.length === 0) { window.showToast('Nichts ausgewählt.'); return; }
        ...
        for (const c of chosen) {
            ...
            if (mode === 'machine') {
                ...
                if (isNew && !(c.querySelector('.ai-cap-machine')?.value)) {
                    ...
                    window.showToast('Bitte ordne der Aufgabe eine Maschine zu – oder wechsle auf „Werkstattauftrag". Gehört sie zu nichts, entferne den Haken.');
                    return;
                }
            } else {
                ...
                if (isNew && !(c.querySelector('.ai-cap-wa-number')?.value.trim())) {
                    ...
                    if (hint) hint.style.display = 'block';
                    window.showToast('Bitte gib die Werkstattauftragsnummer ein.');
                    return;
                }
            }
        }

        if (!window.supabaseClient) { window.showToast('Keine Datenbankverbindung.'); return; }
...
- Bekannte Mitarbeiter (für assignee_hint, exakt so schreiben, mehrere mit Komma): ${userNames || 'keine'}.`;
    ...
    function renderSrPreview() {
        ...
        const aUserIds = matchUserIds(r.assignee_hint);
        ...
        let html = `
            ...
            <div style="margin-bottom:0.75rem;">
                <div style="font-size:0.72rem; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:1px; margin-bottom:4px;">👤 Zuständig (Mehrfachauswahl)</div>
                ${assigneeControl('ai-sr-assignee', aUserIds)}
            </div>
    ...
    window.applyAiServiceReport = function () {
        ...
        if (typeof window.openServiceberichtModal !== 'function') {
            window.showToast('Servicebericht-Modal ist nicht verfügbar.');
            return;
        }
    ...
    window.aiCapSuggestStepsForCard = async function (i) {
        ...
        if (!body.trim() && !title.trim()) { window.showToast('Kein Text vorhanden, aus dem Schritte abgeleitet werden könnten.'); return; }
        ...
        try {
            ...
            if (newSteps.length === 0) {
                window.showToast('Die KI hat keine sinnvollen Schritte für diesen Text gefunden.');
                return;
            }
