                        ...
                        ${subtasks.map((st, sIdx) => `
                            <div style="display: flex; align-items: center; gap: 12px; background: rgba(255,255,255,0.03); padding: 8px 12px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.05);">
                                ...
                                <div style="flex: 1; display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <span onclick="window.setSubtaskAction(null, '${st.action_type || ''}', { taskModal: true, gIdx: ${gIdx}, sIdx: ${sIdx} })" 
                                          style="font-size: 0.65rem; cursor: pointer; background: ${st.action_type ? (st.action_type.startsWith('document:') ? 'rgba(16, 185, 129, 0.2)' : (st.action_type === 'intake' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(245, 158, 11, 0.2)')) : 'rgba(255,255,255,0.05)'}; color: ${st.action_type ? (st.action_type.startsWith('document:') ? '#10b981' : (st.action_type === 'intake' ? '#60a5fa' : '#f59e0b')) : 'rgba(255,255,255,0.4)'}; padding: 2px 5px; border-radius: 4px; font-weight: 800; border: 1px solid ${st.action_type ? (st.action_type.startsWith('document:') ? 'rgba(16, 185, 129, 0.3)' : (st.action_type === 'intake' ? 'rgba(59, 130, 246, 0.3)' : 'rgba(245, 158, 11, 0.3)')) : 'rgba(255,255,255,0.1)'}; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                        ${st.action_type ? (st.action_type.startsWith('document:') ? '📄 ' + (st.action_type.substring(9).split('|||')[1] || 'Dokument') : (st.action_type === 'intake' ? '⚡ Eingang' : '⚡ Endcheck')) : '+ Aktion'}
                                    </span>
                                </div>
